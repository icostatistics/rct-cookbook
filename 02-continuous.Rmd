...
# test1 <- rct %>% 
#   filter(time==3) %>%
#   lm(contout ~ trt + site, data=.) %>%
#   prediction(at = list(trt = c("Active", "Placebo")))
# 
# #tmp <- car::deltaMethod(test1, "Intercept + trtActive + site2 + covar + contbl")
# 
#   test2 <- prediction(test1,data = rct,at = list(trt = c("Active", "Placebo"))) %>%
#     as_tibble %>%
#     filter(time == 3) %>%
#     
#   
#   test2 %>%
#     group_by(trt) %>%
#     summarise(mean = mean(fitted), se = median(se.fitted), n=n())
#   
#  test1 %>%
#    ref_grid() %>%
#   emmeans( ~ trt)
